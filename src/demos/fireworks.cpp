/************************************************************************
 * PURELY FOR DEMONSTRATION OF PARTICLE PHYSICS, NOT IMPORTANT TO PROJECT
 ************************************************************************/
...
static marballs::Random crandom;

/**
 * Fireworks are particles, with additional data for rendering and
 * evolution.
 */
class Firework : public marballs::Particle
{
public:
     ...
     * removed.
     */
    bool update(marballs::marb duration)
    {
...
struct FireworkRule
{
     ...
     */
    void SetParameters(unsigned type, marballs::marb minAge, marballs::marb maxAge,
        const marballs::Vector3 &minVelocity, const marballs::Vector3 &maxVelocity,
        marballs::marb damping)
    {
    ...
    void create(Firework *firework, const Firework *parent = NULL) const
    {
        ...
        {
            marballs::Vector3 start;
            int x = (int)crandom.randomInt(3) - 1;
            start.x = 5.0f * marballs::marb(x);
        ...
        firework->SetAcceleration(marballs::Vector3::GRAVITY);
...
void FireworksDemo::initFireworkRules()
{
    ...
    rules[0].SetParameters(
        1, // type
        0.5f, 1.4f, // age range
        marballs::Vector3(-5, 25, -5), // min velocity
        marballs::Vector3(5, 28, 5), // max velocity
        0.1 // damping
        );
    ...
    rules[1].SetParameters(
        2, // type
        0.5f, 1.0f, // age range
        marballs::Vector3(-5, 10, -5), // min velocity
        marballs::Vector3(5, 20, 5), // max velocity
        0.8 // damping
        );
    ...
    rules[2].SetParameters(
        3, // type
        0.5f, 1.5f, // age range
        marballs::Vector3(-5, -5, -5), // min velocity
        marballs::Vector3(5, 5, 5), // max velocity
        0.1 // damping
        );
    ...
    rules[3].SetParameters(
        ...
        marballs::Vector3(-20, 5, -5), // min velocity
        marballs::Vector3(20, 5, 5), // max velocity
        0.2 // damping
        );
    ...
    rules[4].SetParameters(
        5, // type
        0.5f, 1.0f, // age range
        marballs::Vector3(-20, 2, -5), // min velocity
        marballs::Vector3(20, 18, 5), // max velocity
        0.01 // damping
        );
    ...
    rules[5].SetParameters(
        ...
        3, 5, // age range
        marballs::Vector3(-5, 5, -5), // min velocity
        marballs::Vector3(5, 10, 5), // max velocity
        0.95 // damping
        );
    ...
    rules[6].SetParameters(
        7, // type
        4, 5, // age range
        marballs::Vector3(-5, 50, -5), // min velocity
        marballs::Vector3(5, 60, 5), // max velocity
        0.01 // damping
        );
    ...
    rules[7].SetParameters(
        8, // type
        0.25f, 0.5f, // age range
        marballs::Vector3(-1, -1, -1), // min velocity
        marballs::Vector3(1, 1, 1), // max velocity
        0.01 // damping
        );
    ...
    rules[8].SetParameters(
        9, // type
        3, 5, // age range
        marballs::Vector3(-15, 10, -5), // min velocity
        marballs::Vector3(15, 15, 5), // max velocity
        0.95 // damping
        );
...
{
    const static marballs::marb size = 0.1f;
        ...
        firework++)
    {
        ...
        if (firework->type > 0)
        {
            ...
            const marballs::Vector3 &pos = firework->GetPosition();
